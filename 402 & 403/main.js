// INITIALISE MORALIS
(async function(){
    const serverUrl = "https://0fndufgtrv4e.usemoralis.com:2053/server"
    const appId = "Bw6kFRxqlJaChjLLwVmvueRPlZCZm6stQnCGr0y8"
...
getNFTs = async () => {
    let _chain = document.querySelector('#getNFTs-chain').value;
    let _address = document.querySelector('#getNFTs-address').value;
    let nfts = await Moralis.Web3API.account.getNFTs({ chain: _chain, address: _address });
...
getNFTTransfers = async () => {
    let _chain = document.querySelector('#getNFTTransfers_chain').value;
    let _address = document.querySelector('#getNFTTransfers_address').value;
    let _offset = document.querySelector('#getNFTTransfers_offset').value;
    let _order = document.querySelector('#getNFTTransfers_order').value;
    let _limit = document.querySelector('#getNFTTransfers_limit').value;
    let _format = document.querySelector('#getNFTTransfers_format').value;
    let _direction = document.querySelector('#getNFTTransfers_direction').value;

    const options = {
        chain: _chain,
        address: _address,
        offset: _offset ? _offset : undefined,
        order: _order ? _order : undefined,
        limit: _limit ? _limit : undefined,
        format: _format ? _format : undefined,
        direction: _direction ? _direction : undefined,
    }
    console.log(options)
    let transferNFT = await Moralis.Web3API.account.getNFTTransfers(options);

    let content = document.querySelector('#getNFTTransfers_content');
    content.innerHTML = "";
    console.log(transferNFT)

    if (transferNFT.result.length > 0) {
        transferNFT.result.forEach(result =>{
            let firstContent = document.createElement('div')
            firstContent.className = "card col-md-4 my-4 transfersNFT"
            ...
            Object.keys(result).forEach(key => {
                ...
                firstContent.appendChild(div);
            });
            content.appendChild(firstContent);
        })
    } else content.innerHTML += `<div class="container text-center my-3 h3">You have no public NFTs on the ${_chain}
    chain </div>`;

}

getNFTsForContract = async () => {
    let _chain = document.querySelector('#getNFTsForContract_chain').value;
    let _address = document.querySelector('#getNFTsForContract_address').value;
    let _offset = document.querySelector('#getNFTsForContract_offset').value;
    let _order = document.querySelector('#getNFTsForContract_order').value;
    let _limit = document.querySelector('#getNFTsForContract_limit').value;
    let _tokenAddress = document.querySelector('#getNFTsForContract_tokenAddress').value;
    ...
    const options = {
        ...
        order: _order ? _order : undefined,
        limit: _limit ? _limit : undefined,
        token_address: _tokenAddress ? _tokenAddress : undefined,
    }
    console.log(options)
    let theNFTdata = await Moralis.Web3API.account.getNFTsForContract(options);
    console.log(theNFTdata)

    let content = document.querySelector('#getNFTsForContract_content');
    content.innerHTML = "";

    if (theNFTdata.result.length > 0) {
        theNFTdata.result.forEach(result =>{
            let firstContent = document.createElement('div')
            firstContent.className = "card col-md-4 my-4 transfersNFT"
...
getTokenPrice = async (token) => {
    ...
    const price = await Moralis.Web3API.token.getTokenPrice({address: token});
    
    const content = document.getElementById('getTokenPrice_content')
    content.innerHTML="";
    const h4 = document.createElement('h4');
    h4.innerText = `$${price.usdPrice.toFixed(5)}`
    content.appendChild(h4);

}

getAllTokenIds = async (token) => {

    const _chain = document.getElementById('getTokenIds-chain').value
    const _address = document.getElementById('getAllTokenIds-address').value

    const options = { address: _address, chain: _chain };
    const NFTs = await Moralis.Web3API.token.getAllTokenIds(options);
    console.log(NFTs)

}
// WEB3API TOKEN FUNCTIONS

getTokenMetadata = async (token) => {

    console.log('pressed')
    const _chain = document.getElementById('getTokenMetadata-chain').value
    const _addresses = document.getElementById('getAllTokenMetadata-addresses').value
    
    const options = { chain: _chain, addresses: _addresses };
    console.log(options)
...
getTokenMetadataBySymbol = async (token) => {

    const _chain = document.getElementById('getTokenMetadataBySymbol-chain').value
    const _symbols = document.getElementById('getTokenMetadataBySymbol-symbols').value
    
    const options = { chain: _chain, symbols: _symbols };
    const metadata = await Moralis.Web3API.token.getTokenMetadataBySymbol(options);
...
resolveDomain = async (domain) => {
    const content = document.getElementById('resolveDomain-content')
    content.innerHTML = ""
    const div = document.createElement('h4')
    if(!domain){return content.innerHTML = "<h4>enter a domain</h4>"}

    if(domain.endsWith(".eth")){
        try {
            window.web3 = await Moralis.enableWeb3()
            const resolve = await window.web3.eth.ens.getAddress(domain)
            div.innerText = `${domain}: ${resolve}`;
            content.appendChild(div);
        } catch (error) {
            div.innerText = `nope, couldn't resolve ${domain}, try another`;
            content.appendChild(div);
        }
    }
    else {
        try {
            const options = { currency: 'eth', domain: domain};
            const resolve = await Moralis.Web3API.resolve.resolveDomain(options);
            div.innerText = `${domain}: ${resolve.address}`;
            content.appendChild(div);
        } catch (error) {
            div.innerText = `nope, couldn't resolve ${domain}, try another`;
            content.appendChild(div);
...
if (window.location.pathname.endsWith(dashboard)){
    ...
    document.querySelector('#btn_getTokenMetadata').onclick = getTokenMetadata;
    document.querySelector('#btn_getTokenMetadataBySymbol').onclick = getTokenMetadataBySymbol;
    document.querySelector('#btn_getTokenPrice').onclick = (() => getTokenPrice(document.getElementById('getTokenPrice-address').value));
